...
docker run -it -v C:\Code\deep-learning-keras-tf-tutorial\28_tf_serving:/28_tf_serving -p 8601:8601 --entrypoint /bin/bash tensorflow/serving


To serve only latest model
tensorflow_model_server --rest_api_port=8601 --model_name=email_model --model_base_path=/28_tf_serving/saved_models/

To serve models using model config file
tensorflow_model_server --rest_api_port=8601  --allow_version_labels_for_unavailable_models --model_config_file=/28_tf_serving/model.config.c
